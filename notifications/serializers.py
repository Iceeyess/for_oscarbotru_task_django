...
class NotificationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        delay = {0: datetime.datetime.now(timezone(TIME_ZONE)), 1: datetime.datetime.now(timezone(TIME_ZONE))
                 + datetime.timedelta(hours=1), 2: datetime.datetime.now(timezone(TIME_ZONE))
